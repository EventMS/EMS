...
namespace EMS.Subscription_Services.API.UnitTests.GraphQL
{
    ...
    class ClubMutationsTests : BaseMutationsSetupTests<ClubContext>
    {
        ...
        [Test]
        public async Task AddInstructor_Valid_ThrowsEventAsExpected()
        {
            var request = UpdateClubRequest();
            var clubCreated = _mapper.Map<Club>(CreateClubRequest());
            clubCreated.AdminId = _currentUser.UserId;
            ...
            await _mutations.AddInstructorAsync(clubCreated.ClubId, Guid.NewGuid());
            await _publish.Received(1).Publish(Arg.Any<IsUserClubMemberEvent>());
        }

        [Test]
        public async Task AddInstructor_Clubdoesnotexist_failsd()
        {
            var request = UpdateClubRequest();
            var clubCreated = _mapper.Map<Club>(CreateClubRequest());
            clubCreated.AdminId = _currentUser.UserId;
            ...
            Assert.ThrowsAsync<QueryException>(async () =>
                await _mutations.AddInstructorAsync(Guid.NewGuid(), Guid.NewGuid()));
            await _publish.Received(0).Publish(Arg.Any<IsUserClubMemberEvent>());
        }


        [Test]
        public async Task AddInstructor_Invalid_Fails()
        {
            Assert.ThrowsAsync<QueryException>(async () =>
                await _mutations.AddInstructorAsync(Guid.NewGuid(), Guid.NewGuid()));
            await _publish.Received(0).Publish(Arg.Any<IsUserClubMemberEvent>());
        ...
        [Test]
        public async Task RemoveInstructor_Valid_ThrowsEventAsExpected()
        {
            var request = UpdateClubRequest();
            var clubCreated = _mapper.Map<Club>(CreateClubRequest());
            clubCreated.AdminId = _currentUser.UserId;
            clubCreated.InstructorIds = new HashSet<Guid>();
            clubCreated.InstructorIds.Add(Guid.NewGuid());
            using (var context = _factory.CreateContext())
            {
                context.Clubs.Add(clubCreated);
                context.SaveChanges();
            ...
            await _mutations.RemoveInstructorAsync(clubCreated.ClubId, clubCreated.InstructorIds.First());
            await _publish.Received(1).Publish(Arg.Any<InstructorDeletedEvent>());
        }

        [Test]
        public async Task RemoveInstructor_invalidclub_ThrowsEventAsExpected()
        {
            Assert.ThrowsAsync<QueryException>(async () =>
                await _mutations.RemoveInstructorAsync(Guid.NewGuid(), Guid.NewGuid()));
            await _publish.Received(0).Publish(Arg.Any<InstructorDeletedEvent>());
        }


        [Test]
        public async Task RemoveInstructor_invalidInstructor_ThrowsEventAsExpected()
        {
            var request = UpdateClubRequest();
            var clubCreated = _mapper.Map<Club>(CreateClubRequest());
            clubCreated.AdminId = _currentUser.UserId;
            ...
            await _mutations.RemoveInstructorAsync(clubCreated.ClubId, Guid.NewGuid());
            await _publish.Received(0).Publish(Arg.Any<InstructorDeletedEvent>());
        }
    }
}
